...
class Super_User:
    ...
    def create_member(self,name,age, gender, mobile_num, email, BMI, membership_dur):
        ...
        if membership_dur==3 or membership_dur==1 or membership_dur==6 or membership_dur==12:
            ...
            self.member[mobile_num]["membership_dur"] = membership_dur
        
    def view_member(self):
        for i in self.member.keys():
            print("Name :",self.member[i]["name"])
            print("\nage :",self.member[i]["age"])
            print("\nGender :",self.member[i]["gender"])
            print("\nMobile Numer :",self.member[i]["mobile_num"])
            print("\nEmail :",self.member[i]["email"])
            print("\nBMI :",self.member[i]["BMI"])
            print("\nMembership Duration :",self.member[i]["membership_dur"])
